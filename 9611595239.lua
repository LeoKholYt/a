...
runcode(function()
	...
	RunLoops:BindToHeartbeat("PositionUpdater", 1, function()
		for i, v in pairs(getAttackable()) do
			brvelo[v.Player.Name] = v.Character.Head.Position - (brpos[v.Player.Name] or v.Character.Head.Position)	
			brpos[v.Player.Name] = v.Character.Head.Position
		end
		if uninjectflag then
			RunLoops:UnbindFromHeartbeat("PositionUpdater")
		end
	end)
end)
			...
			if callback then
				...
				debug.setupvalue(br["CrosshairController"].getAimPosition, 5, {
					raycast = function(self, origin, direction, args, ...)
						...
						currentplr = plr
						if plr then 
							local newvelo = Vector3.new(brvelo[plr.Player.Name].X * 1.475, brvelo[plr.Player.Name].Y * 0.9, brvelo[plr.Player.Name].Z * 1.475)
							direction = CFrame.lookAt(origin, plr.Character.Head:GetRenderCFrame().p + newvelo).lookVector * 1000
						end
		...
		["Function"] = function(callback) 
			if callback then 
				task.spawn(function()
					repeat
						task.wait()
						if SilentAim["Enabled"] and mouse1click and (isrbxactive and isrbxactive() or iswindowactive and iswindowactive()) then
							if currentplr then 
								if shoottime <= tick() then
            ...
            if callback then
                ...
                RunLoops:BindToRenderStep("ItemESP", 500, function()
					for i,plr in pairs(itemsnear) do 
						...
						local headPos, headVis = cam:WorldToViewportPoint(plr.Position + Vector3.new(0, 5, 0))
						
						if headVis then
							local item = plr:GetAttribute("displayItem")
			...
			if callback then
				task.spawn(function()
					repeat
						...
						if item then 
							if health.health <= (health.maxHealth - (healthrequirement >= health.maxHealth and health.maxHealth / 2 or healthrequirement)) then 
						...
						if item2 then 
							if health.shield <= (health.maxShield - (requirement >= health.maxShield and health.maxShield / 2 or requirement)) then 
...
runcode(function()
	...
	heallabel.TextSize = 20
	heallabel.Font = Enum.Font.SourceSans
	heallabel.TextColor3 = Color3.new(1, 0.1, 0.1)
	heallabel.TextStrokeTransparency = 0
	heallabel.TextStrokeColor3 = Color3.new()
	heallabel.Position = UDim2.new(0.5, 0, 0.5, 0)
	heallabel.Text = ""
	heallabel.Parent = GuiLibrary["MainGui"]
						...
						if plr then 
							...
							if royaleentity and royaleentity:GetAttribute("downed") then
								if reviving ~= plr.Player then 
									reviving = plr.Player
									revivetick = tick() + 6
									heallabel.Text = "Healing: "..(plr.Player.DisplayName or plr.Player.Name).." in 6"
									br["Networking"].NetEvents.client.reviveStarted:fire(plr.Player.UserId)
								else
									heallabel.Visible = true
									heallabel.Text = "Healing: "..(plr.Player.DisplayName or plr.Player.Name).." in "..math.floor((revivetick - tick()) * 10) / 10
									if revivetick <= tick() then 
										heallabel.Text = "Healed: "..(plr.Player.DisplayName or plr.Player.Name)
...
runcode(function()
	...
	heallabel.TextSize = 20
	heallabel.Font = Enum.Font.SourceSans
	heallabel.TextColor3 = Color3.new(0.1, 1, 0.1)
	heallabel.TextStrokeTransparency = 0
	heallabel.TextStrokeColor3 = Color3.new()
	heallabel.Position = UDim2.new(0.5, 0, 0.5, 20)
	heallabel.Text = ""
	heallabel.Parent = GuiLibrary["MainGui"]
							...
							for i,v in pairs(collectionservice:GetTagged("AirdropPackage")) do 
								if v.PrimaryPart and (localchar.HumanoidRootPart.Position - v.PrimaryPart.Position).Magnitude <= 12 then 
									...
									if reviving ~= v then 
										reviving = v
										revivetick = tick() + 3
										heallabel.Text = "Opening Airdrop in 3"
									else
										heallabel.Visible = true
										heallabel.Text = "Opening Airdrop in "..math.floor((revivetick - tick()) * 10) / 10
										if revivetick <= tick() then 
											br["Networking"].NetEvents.client.openAirdropPackage(v)
											heallabel.Text = "Opening Airdrop"
										end
									end
									break
								end
